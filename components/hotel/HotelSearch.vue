<template>
	<div class="container-fluid search" :class="homeSearch? 'home-page' : ''">
		<div class="home-page__container d-flex">
			<p v-if="cityInputIsEmpty" :class="homeSearch ? 'home-empty-input-text' : 'empty-input-text' ">
				لطفا شهر یا هتل مورد نظر خود را انتخاب کنید!
			</p>
			<div id="search-box" :class="{ 'hotel-search-box' : !homeSearch }">
				<div class="search-box-border-left search-box-border-bottom city-box-search d-flex align-items-center" :class="{'empty-input': cityInputIsEmpty}">
					<city-input
						ref="origin"
						class="w-100"
						:input-title="''"
						...
						@setValue="setValue"
						@checkInpputIsEmptiy="checkInpputIsEmptiy"
						@closeDatePicker="closeDatePicker"
					>
						<template #default="{item}">
							<div class="d-flex">
								<div>
									<img v-if="item.type == 'Hotel' " src="~assets/images/hotel-icon.svg" alt="hotel-image" class="ml-3 hotel-list-icon city-icon">
								...
								<div class="d-flex flex-column">
									...
									<div v-if="item.province" class="state">
										<span v-if="item.cityName">شهر {{ item.cityName }} ,</span>
										استان {{ item.province }}
									</div>
								</div>
							</div>
				...
				<div v-if="homeSearch" class="submit-box mr-auto">
					...
					>
						<div class="select mr-auto">
							<div
								class="selected d-flex align-items-center"
								:class="{open: open}"
								@click="open = !open"
							>
								...
								<img src="~assets/images/angle-down.svg" class="capacity-angle-down mt-1 d-none" alt="angle down">
							</div>
							<div
								class="items"
								:class="{selectHide: !open}"
							>
								<div v-for="(capacity, index) in capacityOptions" :key="index" class="item" @click="setSelectedCapacity(capacity.value); open=false;">
									{{ capacity.label }}
								</div>
							</div>
						</div>
						<div class="mobile-select mr-auto d-none">
							...
							<div v-if="open" class="capacity-mobile-wrapper" @click.self="open = false">
								<div class="capacity-mobile">
									<span class="capacity-mobile__title">ظرفیت اتاق را انتخاب کنید</span>
									<div v-for="(capacity, index) in mobileCapacityOptions" :key="index" class="capacity-mobile__item d-flex align-items-center" @click="setSelectedCapacity(capacity.value); open=false;">
										<span class="capacity-mobile__item__checkbox" :class="{'capacity-mobile__item__selected-checkbox': selectedCapacity === capacity.value}">
		...
		<transition name="fade">
			<p v-if="cityInputIsEmpty" class="d-none empty-input-warning">
				<svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
					<path d="M17.7188 9.27075C17.7188 4.4895 13.7812 0.552002 9 0.552002C4.18359 0.552002 0.28125 4.4895 0.28125 9.27075C0.28125 14.0872 4.18359 17.9895 9 17.9895C13.7812 17.9895 17.7188 14.0872 17.7188 9.27075ZM9 11.0286C9.87891 11.0286 10.6172 11.7668 10.6172 12.6458C10.6172 13.5598 9.87891 14.2629 9 14.2629C8.08594 14.2629 7.38281 13.5598 7.38281 12.6458C7.38281 11.7668 8.08594 11.0286 9 11.0286ZM7.45312 5.22778C7.41797 4.98169 7.62891 4.77075 7.875 4.77075H10.0898C10.3359 4.77075 10.5469 4.98169 10.5117 5.22778L10.2656 10.009C10.2305 10.2551 10.0547 10.3958 9.84375 10.3958H8.12109C7.91016 10.3958 7.73438 10.2551 7.69922 10.009L7.45312 5.22778Z" fill="#EB384D" />
				</svg>
...
export default {
	...
	computed: {
		selectedDestination() {
			return this.$route.params.hotel ? this.$route.params.origin + '/' + this.$route.params.hotel : (this.$route.params.origin ? this.$route.params.origin : '');
		}
	},
	watch: {
	...
	methods: {
		...
		searchOrEdit: function () {
			...
			setTimeout(function () {
				that.$emit(
					'searchHotels',
					that.$refs.origin.id.toLowerCase().replace(/ /g, '-'),
					dates.$refs.datepicker.dates.start.getFullYear() +
            '-' +
            that.pad(dates.$refs.datepicker.dates.start.getMonth() + 1) +
            '-' +
            that.pad(dates.$refs.datepicker.dates.start.getDate()),
					dates.$refs.datepicker.dates.end.getFullYear() +
            '-' +
            that.pad(dates.$refs.datepicker.dates.end.getMonth() + 1) +
            '-' +
      that.pad(dates.$refs.datepicker.dates.end.getDate()),
					that.selectedCapacity
				);
			}, 500);
		},
		setValue: function () {
